import React from "react";
import "./Philosphy.css";
const Philosphy = () => {
  return (
    <>
      {/* <div className="card-cont">
        <div className="card-title1">
          <p>
            Our <span id="para">Philosphy</span>
          </p>
          <p>
            Journey to Enlightenment: Unraveling the Spiritual Tapestry of
            Islamic Philosophy.
          </p>
        </div>
        <div className="card-cont1">
          <div className="card">
            <img
              src="../../../public/Images/Quran.png"
              alt="Card"
              className="card-image"
            />
            <div className="card-content">
              <h2 className="card-title">Online Learning</h2>
              <p className="card-text">
                Quran Facility refers to the memorization and recitation of the
                Quran.Which is the holy scripture of Quran
              </p>
            </div>
          </div>
          <div className="card">
            <img
              src="../../../public/Images/star.png"
              alt="Card"
              className="card-image"
            />
            <div className="card-content">
              <h2 className="card-title">Spirituality</h2>
              <p className="card-text">
              Quran Facility refers to the memorization and recitation of the
                Quran.Which is the holy scripture of Quran
              </p>
            </div>
          </div>
          <div className="card">
            <img
              src="../../../public/Images/prayer.png"
              alt="Card"
              className="card-image"
            />
            <div className="card-content">
              <h2 className="card-title">Service</h2>
              <p className="card-text">
              Quran Facility refers to the memorization and recitation of the
                Quran.Which is the holy scripture of Quran
              </p>
            </div>
          </div>
          <div className="card">
            <img
              src="../../../public/Images/Child.png"
              alt="Card"
              className="card-image"
            />
            <div className="card-content">
              <h2 className="card-title">Community</h2>
              <p className="card-text">
              Quran Facility refers to the memorization and recitation of the
                Quran.Which is the holy scripture of Quran
              </p>
            </div>
          </div>
          <div className="card">
            <img
              src="../../../public/Images/masjid-card.png"
              alt="Card"
              className="card-image"
            />
            <div className="card-content">
              <h2 className="card-title">Certificate</h2>
              <p className="card-text">
              Quran Facility refers to the memorization and recitation of the
                Quran.Which is the holy scripture of Quran
              </p>
            </div>
          </div>
          <div className="card">
            <img
              src="../../../public/Images/globe.png"
              alt="Card"
              className="card-image"
            />
            <div className="card-content">
              <h2 className="card-title">Learn from Anywhere Anytime</h2>
              <p className="card-text">
              Quran Facility refers to the memorization and recitation of the
                Quran.Which is the holy scripture of Quran
              </p>
            </div>
          </div>
        </div>
      </div> */}
        <div className="philosphy-container">
          <h1 className="our-philosophy">Our <span>Philosophy</span></h1>
          <p id="phi-para">Journey to Enlightment: Unraveling the Spiritual Tapestry of Islamic Philosophy</p>
          <div className="row">
            <div className="col">
              <div className="img-icon">
                <img src="../../../public/Images/Quran.png" alt="" srcset="" />
              </div>
              <div className="philosophy-title">
                <h1>Online Learning</h1>
                <p>Quran Facility refers to the memorization and recitation of the Quran.Which is the holy Scripture of the Islam</p>
              </div>
            </div>
            <div className="col">
              <div className="img-icon">
                <img src="../../../public/Images/Star.png" alt="" srcset="" />
              </div>
              <div className="philosophy-title">
                <h1>Spirituality</h1>
                <p>Quran Facility refers to the memorization and recitation of the Quran.Which is the holy Scripture of the Islam</p>
              </div>
            </div>
            <div className="col">
              <div className="img-icon">
                <img src="../../../public/Images/Quran.png" alt="" srcset="" />
              </div>
              <div className="philosophy-title">
                <h1>Service</h1>
                <p>Quran Facility refers to the memorization and recitation of the Quran.Which is the holy Scripture of the Islam</p>
              </div>
            </div>
            <div className="col">
              <div className="img-icon">
                <img src="../../../public/Images/prayer.png" alt="" srcset="" />
              </div>
              <div className="philosophy-title">
                <h1>Community</h1>
                <p>Quran Facility refers to the memorization and recitation of the Quran.Which is the holy Scripture of the Islam</p>
              </div>
            </div>
            <div className="col">
              <div className="img-icon">
                <img src="../../../public/Images/globe.png" alt="" srcset="" />
              </div>
              <div className="philosophy-title">
                <h1>Learn from Anywhere Anytime</h1>
                <p>Quran Facility refers to the memorization and recitation of the Quran.Which is the holy Scripture of the Islam</p>
              </div>
            </div>
            <div className="col">
              <div className="img-icon">
                <img src="../../../public/Images/certificate.png" alt="" srcset="" />
              </div>
              <div className="philosophy-title">
                <h1>Certificate</h1>
                <p>Quran Facility refers to the memorization and recitation of the Quran.Which is the holy Scripture of the Islam</p>
              </div>
            </div>
          </div>
        </div>
    </>
  );
};

export default Philosphy;
